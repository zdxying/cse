// Input fixture for the FreeLB .ur.h generator (csegen): moment.h shapes.
// Parsed only (no Vector/Cell/CELLTYPE definitions needed). This is a compact
// representative subset of the real src/lbm/moment.h structures, chosen to keep
// frontend/classification coverage:
//   - CellType structs with `if constexpr` (single-statement and block)
//   - `.template get<...>()` / `.template hasField<...>()`, `T{}` value-init
//   - Vector lowering (componentwise ops, `clear`, `+=`, `/=`, `latset::c`)
//   - nested triangular loops with a straight-line counter and tensor index
//   - a `T`-returning method with `std::sqrt`
// Numerical correctness against the full moment.h is covered by FreeLB's
// verify_moment.py when a FreeLB checkout is available.
